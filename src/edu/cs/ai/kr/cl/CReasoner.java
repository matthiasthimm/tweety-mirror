...
import edu.cs.ai.kr.*;
import edu.cs.ai.kr.cl.semantics.*;
...
public class CReasoner extends Reasoner {
	...
	/* (non-Javadoc)
	 * @see edu.cs.ai.kr.Reasoner#query(edu.cs.ai.logic.Formula)
	 */
	@Override
	...
	private RankingFunction computeCRepresentation(){		
		...
		edu.cs.ai.math.term.Term targetFunction = null;
		...
		Map<Variable, Term> solution = new edu.cs.ai.math.opt.solver.LpSolve(problem).solve();
	...
	private Statement getAcceptanceConstraint(Conditional cond, Map<PossibleWorld,IntegerVariable> ranks){
		...
		edu.cs.ai.math.term.Term leftSide = null;
		for(PossibleWorld w: ranks.keySet()){
		...
		if(leftSide == null)
			leftSide = new edu.cs.ai.math.term.IntegerConstant(0);
		// construct right side of the inequation
		edu.cs.ai.math.term.Term rightSide = null;
		for(PossibleWorld w: ranks.keySet()){
		...
		// if term is still null then set to constant zero
		if(rightSide == null)
			rightSide = new edu.cs.ai.math.term.IntegerConstant(0);
		// return inequation
