package edu.cs.ai.kr.fol.semantics;
...
public class HerbrandInterpretation extends InterpretationSet<Atom> {
	...
	public static void main(String[] args){
		...
		Constant c4 = new Constant("c4");
		signature.add(c1);
		signature.add(c2);
		signature.add(c3);
		signature.add(c4);
		
		Set<Constant> eqClass = new HashSet<Constant>();
		eqClass.add(c1);
		eqClass.add(c2);
		eqClass.add(c3);
		eqClass.add(c4);
		...
		//Set<Constant> eqClass2= new HashSet<Constant>();
		//eqClass2.add(c1);
		//eqClass2.add(c2);		
		//equivalenceClasses.add(eqClass2);
		
		signature.add(new Predicate("p",2));
		//signature.add(new Predicate("q",1));
		...
		while(!interpretations.isEmpty()){
			HerbrandInterpretation i1 = interpretations.iterator().next();
			count++;
			interpretations.remove(i1);
			Set<HerbrandInterpretation> toBeRemoved = new HashSet<HerbrandInterpretation>();
			for(HerbrandInterpretation i2: interpretations)
				if(i1.isSyntacticallyEquivalent(i2, equivalenceClasses))
					toBeRemoved.add(i2);				
			interpretations.removeAll(toBeRemoved);
		}
		System.out.println("Number of reference worlds: " + count);
