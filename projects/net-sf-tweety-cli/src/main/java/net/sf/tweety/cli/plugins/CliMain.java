...
public class CliMain {
	...
	public static Object configCLI() throws ConfigurationException{
		...
		XMLConfiguration tweetyXmlConfig = new XMLConfiguration(TWEETY_CLI_DEFAULT_CONFIG);
	...
	public static void main(String[] args) {
		...
		Collection<TweetyPlugin> plugins = new LinkedList<TweetyPlugin>(pmu.getPlugins(TweetyPlugin.class));
		...
		for (int i = 0; i< args.length ; i++){
			...
			else if(args[i].equals(ARG__INPUT_FILES) || args[i].equals(ARG__INPUT_FILES_SHORT))
			{
				...
				while (!args[i+1].startsWith("-"))	 
					inFiles += args[i];
				String[] files = inFiles.split("[[A-Z,a-z,0-9]?([.][a-z,0-9]?]?){0,1}");
...
//				}
		...
		for(TweetyPlugin tp : pmu.getPlugins(TweetyPlugin.class)){
			if(tp.getCommand().equalsIgnoreCase(plugin)){
				...
				// each input parameter is checked against the called plugin whether it is valid
				for(ArrayList<String> inparam : collectedparams){	
					for(CommandParameter tpcp : tp.getParameters()){
						if(inparam.get(0).startsWith("-")){
							if(inparam.get(0).equalsIgnoreCase(tpcp.getIdentifier())){
								try {
									tpcp.instantiate(inparam.remove(0).toString());
								} catch (CloneNotSupportedException e) {
									
									e.printStackTrace();
								} catch (NullPointerException e) {
									System.out.println("Your input parameter arguments are somehow wrong:");
									e.printStackTrace();
								}
							}
						} else {
