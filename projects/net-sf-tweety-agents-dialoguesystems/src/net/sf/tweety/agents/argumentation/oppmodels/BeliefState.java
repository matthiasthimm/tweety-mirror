...
public abstract class BeliefState {
	 ...
	 * @param env the environment (gives access to the universal Dung theory and dialogue trace)
	 * @param move a possible move
	 * @return "true" if the given move is legal.
	 */
	private boolean isLegal(GroundedEnvironment env, Set<Argument> move) {		
		/* Moves of size 1 */
		if(move.size() != 1) return false;		
		/* Enforce that all but first move attacks a previous move */
		if(!env.getDialogueTrace().isEmpty() && !env.getPerceivedDungTheory(this.knownArguments).isAttacked(new Extension(move), new Extension(env.getDialogueTrace().getArguments())))
				return false;		
		/* Enforce conflict free moves */
		if(env.getPerceivedDungTheory(this.knownArguments).isAttacked(new Extension(move), new Extension(move))) return false;		
		return true;
	}
	
	/**
	 * Returns the set of possible moves in the given situation.
	 * @param env the environment (gives access to the dialogue trace)
	 * @return the set of possible moves in the given situation.
	 */
	protected Set<ExecutableExtension> getLegalMoves(GroundedEnvironment env){
	 ...
	 */
	protected abstract Pair<Float,Set<ExecutableExtension>> doMove(GroundedEnvironment env, DialogueTrace trace);
	...
	public ExecutableExtension move(GroundedEnvironment env){
		Pair<Float,Set<ExecutableExtension>> move = this.doMove(env, env.getDialogueTrace());
		// for know, just select one action at random
		Random rand = new Random();
		ExecutableExtension[] extensions = (ExecutableExtension[]) move.getSecond().toArray();
